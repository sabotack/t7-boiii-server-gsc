...
SetupShared()
{
    ...
    level.overrideMethods[level.commonFunctions.changeTeam]                        = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getTeamCounts]                     = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getTeamBased]                      = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getMaxClients]                     = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getClientTeam]                     = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getClientKillStreak]               = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.backupRestoreClientKillStreakData] = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.waitTillAnyTimeout]                = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.getXuid]                           = ::NotImplementedFunction;
    level.overrideMethods[level.commonFunctions.isBot]                             = ::NotImplementedFunction;
    ...
    _SetDvarIfUninitialized( level.commonKeys.autoBalance, 0 ); 
...
PlayerSpawnEvents() 
{
    ...
    return; // BOIII specific return otherwise server crashes :(
...
{
    data = [];
    data["mode"] = GetDvarString( level.commonKeys.busMode );
    data["directory"] = GetDvarString( level.commonKeys.busDir );
    data["inLocation"] = level.eventBus.inLocation;
    data["outLocation"] = level.eventBus.outLocation;

    LogDebug( "Bus mode requested" );

    busModeRequest = BuildEventRequest( false, level.eventTypes.getBusModeRequested, "", undefined, data );
    QueueEvent( busModeRequest, level.eventTypes.getBusModeRequested, undefined );

    LogDebug( "Bus mode updated" );

    // BOIII specific fix manually reset sv_iw4madmin_in or server will crash :(
    wait 2;
    SetDvar("sv_iw4madmin_in", "");

    if ( GetDvarString( level.commonKeys.busMode ) == "file" && GetDvarString( level.commonKeys.busDir ) != "" )
    {
...
{
    LogDebug( "Get commands requested" );
...
SendCommands( commandName )
{
    ...
    for ( i = 0; i < level.customCommands.size; i++ )
    {
        ...
        LogDebug( "Sending custom command " + ( i + 1 ) + "/" + level.customCommands.size + ": " + data["name"] );
        commandRegisterRequest = BuildEventRequest( false, level.eventTypes.registerCommandRequested, "", undefined, data );
        // not threading here as there might be a lot of commands to register
        QueueEvent( commandRegisterRequest, level.eventTypes.registerCommandRequested, undefined );
    }
}
...
RegisterScriptCommand( eventKey, name, alias, description, minPermission, supportedGames, requiresTarget, handler )
{
    ...
    {
        LogError( "eventKey must be provided for script command" );
        return;
    }
    ...
    {
        LogError( "name must be provided for script command" );
        return;
    }
    ...
    {
        LogWarning( "handler not defined for script command " + name );
    }
...
RequestUrl( url, method, body, headers, webNotify )
{
    ...
    LogDebug( "next notify index is " + webNotify.index );
    ...
    webNotifyEvent = BuildEventRequest( true, level.eventTypes.urlRequested, "", webNotify.index, data );
    thread QueueEvent( webNotifyEvent, level.eventTypes.urlRequested, webNotify );
...
WaitForUrlRequestComplete()
{
    ...
    {
        LogWarning( "Request to " + self.url  + " timed out" );
        self notify ( level.eventTypes.urlRequestCompleted, "error" );
    }

    LogDebug( "Request to " + self.url  + " completed" );
...
OnUrlRequestCompletedCallback( event )
{
    ...
    {
        LogWarning( "Incomplete data for url request callback. [1]" );
        return;
    }
    ...
    {
        LogWarning( "Incomplete data for url request callback. [2] " + CoerceUndefined( notifyEnt ) + " , " +  CoerceUndefined( response ) );
        return;
    }
    ...
    {
        LogDebug( "Additional data available for url request " + notifyEnt + " (" + event.data["remaining"] + " chunks remaining)" );
        return;
    }
...
OnPlayerDisconnect()
{
    ...
    for ( ;; )
    {
        ...
        LogDebug( "client is disconnecting" );
...
OnPlayerJoinedTeam()
{
    ...
    for( ;; )
    {
        ...
        LogDebug( self.name + " switched to " + newTeam );
        
        if ( newTeam != level.commonKeys.team1 && newTeam != level.commonKeys.team2 )
        {
            OnTeamSizeChanged();
            LogDebug( "not force balancing " + self.name + " because they switched to spec" );
            continue;
        }
...
OnTeamSizeChanged()
{
    ...
    {
        LogDebug( "not enough clients to autobalance" );
        return;
    }

    if ( !IsDefined( GetSmallerTeam( 1 ) ) )
    {
        LogDebug( "teams are not unbalanced enough to auto balance" );
        return;
    }
    ...
    LogDebug( "swapping " + toSwap.name + " from " + curentTeam + " to " + otherTeam );
...
FindClientToSwap()
{
    ...
    if ( !IsDefined( client ) )
    {
        LogDebug( "could not find candidate to swap teams" );
    }
    else
    {
        LogDebug( "best candidate to swap teams is " + client.name );
    }
...
GetBestSwapCandidate( team )
{
    ...
    for ( i = 0; i < maxClients; i++ )
    {
        ...
        LogDebug( "candidate killstreak is " + candidateKillStreak );
        
        if ( candidateKillStreak > 3 )
        {
            LogDebug( "skipping candidate selection for " + candidate.name + " because their kill streak is too high" );
            continue;
        }

        if ( IsDefined( candidate.autoBalanceCount ) && candidate.autoBalanceCount > 2 )
        {
            LogDebug( "skipping candidate selection for " + candidate.name + " they have been swapped too many times" );
            continue;
        }
...
GetClosestPerformanceClientForTeam( sourceTeam, excluded )
{
    ...
    for ( i = 0; i < players.size; i++ )
    {
        ...
        LogDebug( "perf=" + clientPerformance + " candidateValue=" + candidateValue + " src=" + sourceTeamNewPerformance + " dst=" + otherTeamNewPerformance );
        ...
        {
            LogDebug( candidateValue + " is the new best value " );
            choice = players[i];
            closest = candidateValue;
        }
    }

    LogDebug( choice.name + " is the best candidate to swap" + " with closest=" + closest );
    return choice;
}
...
GetTeamToJoin()
{
    ...
    {
        LogDebug( "Team1 performance is lower, so selecting Team1" );
        return level.commonKeys.team1;
    }
    
    else
    {
        LogDebug( "Team2 performance is lower, so selecting Team2" );
...
GetSmallerTeam( minDiff )
{
    ...
    {
        LogDebug( "Team2 is full, so selecting Team1" );
        return level.commonKeys.team1;
    }
    
    if ( team1Count == maxClients / 2 )
    {
        LogDebug( "Team1 is full, so selecting Team2" );
        return level.commonKeys.team2;
    }
    ...
    {
        LogDebug( "Team size differs by more than 1" );
        
        if ( team1Count < team2Count )
        {
            LogDebug( "Team1 is smaller, so selecting Team1" );
            return level.commonKeys.team1;
        }
        
        else
        {
            LogDebug( "Team2 is smaller, so selecting Team2" );
            return level.commonKeys.team2;
        }
    }
