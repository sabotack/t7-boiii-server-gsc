...
SetupBase()
{
    ...
    level.overrideMethods[level.commonFunctions.setDvar]                = ::NotImplementedFunction;
    ...
    {
        level.testdebugvar = "GAMEINTERFACE IS NOT ENABLED!";
        return;
    }
    level.testdebugvar = "GAMEINTERFACE IS ENABLED!";
...
{
    assert( clientNum >= 0, "clientNum cannot be negative" );
...
BuildEventRequest( responseExpected, eventType, eventSubtype, entOrId, data ) 
{
    ...
    groupSeparator = GetSubStr( GetDvarString( "GroupSeparatorChar" ), 0, 1 );
...
MonitorBus()
{
    ...
    level.eventBus.inLocation = level.eventBus.inVar + "_" + GetDvarString( "net_port" );
    level.eventBus.outLocation = level.eventBus.outVar + "_" + GetDvarString( "net_port" );
...
ParseDataString( data ) 
{
    ...
    dataParts = strtok( data, GetSubStr( GetDvarString( "RecordSeparatorChar" ), 0, 1 ) );
    dict = [];

    for ( i = 0; i < dataParts.size; i++ )
    {
        part = dataParts[i];
        splitPart = strtok( part, GetSubStr( GetDvarString( "UnitSeparatorChar" ), 0, 1 ) );
        key = splitPart[0];
        value = splitPart[1];
        dict[key] = value;
        dict[i] = key;
...
BuildDataString( data )
{
    ...
    unitSeparator = GetSubStr( GetDvarString( "UnitSeparatorChar" ), 0, 1 );
    recordSeparator = GetSubStr( GetDvarString( "RecordSeparatorChar" ), 0, 1 );
...
{
    assert( isDefined( self ), "player entity is not defined");
